...
class PolygonEnvironment:
    ...
    def draw_env(self, q=None, show=True, draw_goal=True):
        '''
    ...
    def draw_plan(self, plan, planner, dynamic_tree=False, dynamic_plan=True, show=True, fname='plot.jpg'):
        '''
        ...
        if planner is not None:
            ...
            for i, e in enumerate(edges):
                ...
                if dynamic_tree:
                    plotter.pause(0.001)
            if planner.Tb is not None:
                Qs2, edges2 = planner.Tb.get_states_and_edges()
                # Draw tree for each of the robot links
        ...
        if plan is not None:
            ...
            self.robot.draw(plan[0], color='c')
        plotter.savefig(fname)

    def draw_plan_prm(self, plan, planner, draw_goal=False, dynamic_tree=False, dynamic_plan=True, show=True, fname='plot.jpg'):
        '''
        Draw the environment with an overlaid plan.
        plan - sequence of configurations to be drawn as plan (not drawn if pass in None)
        planner - a planner which has a function of the form
                  ...
                  if None the search graph is not drawn
        '''
        self.draw_env(show=False, draw_goal=draw_goal)
        ...
        if planner is not None:
            Qs, edges = planner.M.get_states_and_edges()
            # Draw tree for each of the robot links
            for i, e in enumerate(edges):
                X0 = self.robot.fk(e[0])
                X1 = self.robot.fk(e[1])
            ...
            Qn = planner.M.nodes
            if draw_goal:
                for j in planner.M.start.connection_paths:
                    xplot = []
                    yplot = []
                    #print j
                    for st in j:
                        ...
                        xplot.append(f0[0])
                        yplot.append(f0[1])
                    plotter.plot(xplot, yplot, 'b')
            for i, nd in enumerate(Qn):
                for j in nd.connection_paths:
                    xplot = []
                    yplot = []
                    #print j
                    for st in j:
